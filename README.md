...
## Поиск по описанию и подсчет категорий
Проект поддерживает поиск по описанию с помощью регулярных выражений и подсчет категорий операций.
### Поиск по описанию
Функция `search_by_description` позволяет искать в списке словарей операций по заданной строке в описании.
### Подсчет категорий
Функция `count_categories` подсчитывает количество банковских операций определенного типа.
### Основная логика проекта
Функция `main` в модуле `main.py` отвечает за основную логику проекта и связывает функциональности между собой.